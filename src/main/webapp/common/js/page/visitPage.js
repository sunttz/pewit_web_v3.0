...
function ajaxCsTable(){
	...
	ajax_jsonp(piwik_url, param, function(data){
		...
		for(var k in data){
			...
			var link = '<a href="historyTrend.html?siteId='+idSite+'&t='+t+'&href='+url+'" title="查看历史趋势"><span class="glyphicon glyphicon-chevron-right"></span></a>&nbsp;&nbsp;&nbsp;<a href="pageUpDown.html?siteId='+idSite+'&t='+t+'&startDate='+startDate+'&endDate='+endDate+'&href='+url+'" title="查看页面上下游"><span class="glyphicon glyphicon-random"></span></a>';
			csData.push({url:'<a href="'+url+'" target="_blank">'+cutStr(url,100)+'</a>',link:link,pv:v.nb_hits,uv:v.nb_visits,atop:formatTime(v.avg_time_on_page),br:v.bounce_rate,er:v.exit_rate,atg:v.avg_time_generation});
		}
		initCsTable(csData);
	});
}
